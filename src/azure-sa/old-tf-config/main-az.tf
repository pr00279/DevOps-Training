...
provider "azurerm" {
  ...
  subscription_id                 = "adc0e27b-f4cc-4114-9a33-7200f92047ef"
  environment                     = "public"
  use_msi                         = false
  use_cli                         = true
  use_oidc                        = false
...
resource "azurerm_storage_container" "res-3" {
  ...
  depends_on = [
    # One of azurerm_storage_account.sa,azurerm_storage_account_queue_properties.res-5 (can't auto-resolve as their ids are identical)
  ]
}
